...
from search_app.utils.sbert_model import encode_book_text   # ← SBERT
...
def index_one_book(book_id: int) -> dict:
    """
    Realtime sync 1 sách lên OpenSearch.
    ...
    # ── SBERT Encoding ───────────────────────────────────────────────────────
    # Nhúng nội dung sách thành Vector 768 chiều để lưu vào OpenSearch k-NN index.
    # Thời gian encode ≈ 30-80ms (CPU) / 5ms (GPU). Không block event loop
    # vì indexer đã được gọi từ ThreadPoolExecutor trong Queue Worker.
    sbert_vector = encode_book_text(title, cats, desc)
