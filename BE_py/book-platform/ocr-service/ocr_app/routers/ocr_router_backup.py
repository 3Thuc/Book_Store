...
v2.1 optimizations (3.3 + 5.2):
  - MD5 result cache: ảnh giống nhau (same bytes) → trả ngay <1ms, không chạy lại EasyOCR
  - Performance logging: ghi lại latency, cache hit rate cho monitoring
"""
import hashlib
import logging
import time
...
# ══════════════════════════════════════════════════════════════════════════════
# TỐI Ư˜U 3.3 – MD5 RESULT CACHE cho /search-by-cover
# ══════════════════════════════════════════════════════════════════════════════
# Vấn đề: Người dùng upload cùng 1 ảnh 2 lần → EasyOCR chạy lại ton điện GPU ~3–8s
# Giải pháp: Hash MD5 của bytes ảnh → cache kết quả 1 giờ
# Kết quả: ảnh trùng →3 < 1ms, giảm tải GPU cho EasyOCR
#
# Cấu trúc cache: {md5_hex: (OCRResponse, expire_at, timestamp)}

_ocr_cache: dict[str, tuple[OCRResponse, float]] = {}
_ocr_cache_lock = threading.Lock()
_OCR_CACHE_TTL  = 3600   # 1 giờ (sách hiếm khi thay đổi trong 1h)
_OCR_CACHE_MAX  = 200    # tối đa 200 ảnh cached (~50MB RAM estimate)

# TỐI Ư˜U 5.2 – Performance counters
_perf = {
    "total_requests":     0,
    "cache_hits":         0,
    "visual_hits":        0,
...
def _ocr_cache_get(md5_hex: str) -> OCRResponse | None:
    """Lấy kết quả OCR đã cache. Trả None nếu miss hoặc expired."""
    with _ocr_cache_lock:
        entry = _ocr_cache.get(md5_hex)
        if entry is None:
            return None
        result, expire_at = entry
        if time.time() > expire_at:
            del _ocr_cache[md5_hex]
            return None
        return result


def _ocr_cache_set(md5_hex: str, result: OCRResponse) -> None:
    """Lưu kết quả OCR vào cache. Evict oldest nếu đầy."""
    with _ocr_cache_lock:
        if len(_ocr_cache) >= _OCR_CACHE_MAX and md5_hex not in _ocr_cache:
            oldest = next(iter(_ocr_cache))
            del _ocr_cache[oldest]
        _ocr_cache[md5_hex] = (result, time.time() + _OCR_CACHE_TTL)


def get_ocr_stats() -> dict:
    """Trả về thống kê hiệu suất OCR để monitor."""
    with _perf_lock:
        total = _perf["total_requests"] or 1
        cache_rate = round(_perf["cache_hits"] / total * 100, 1)
        visual_rate = round(_perf["visual_hits"] / total * 100, 1)
        avg_ms = round(_perf["total_latency_ms"] / total, 1)
        with _ocr_cache_lock:
            cached_count = len(_ocr_cache)
        return {
            "total_requests":  _perf["total_requests"],
            "cache_hits":      _perf["cache_hits"],
            "cache_hit_rate":  f"{cache_rate}%",
            "visual_hits":     _perf["visual_hits"],
            "visual_hit_rate": f"{visual_rate}%",
            "ocr_runs":        _perf["ocr_runs"],
            "avg_latency_ms":  avg_ms,
            "cached_images":   cached_count,
            "cache_capacity":  _OCR_CACHE_MAX,
            "uptime_seconds":  round(time.time() - _perf["last_reset"]),
        }
...
async def search_by_cover(
    ...
    - Visual match HIT:   ~0.5ms  (�    if visual_match is not None:
        # 🎯 KHỞP! Trả kết quả ngay không cần OCR
        ...
        with _perf_lock:
            _perf["visual_hits"] += 1
            _perf["total_latency_ms"] += elapsed
        result = OCRResponse(
            success=True,
        ...
        # Cache visual match kết quả cũng (vì ảnh giống hệt → kết quả giống)
        _ocr_cache_set(md5_hex, result)
        return result
...
eason"])
    ...
    # Cache kết quả nếu thành công (tránh chạy lại OCR cho ảnh trùng)
    if final_response.success and search_results:
        _ocr_cache_set(md5_hex, final_response)
        logger.info("📦 MD5 cache SET (%s...) %d kết quả", md5_hex[:8], len(search_results))
