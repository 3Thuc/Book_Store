...
from ocr_app.middleware.rate_limiter import limiter

from ocr_app.config import MAX_IMAGE_BYTES, ADMIN_API_KEY
...
from ocr_app.services.image_similarity_engine import (
    ...
    get_index_stats,
    build_hash_index,
    upsert_book_in_hash_index,
    remove_book_from_hash_index,
)
...
async def _verify_admin_key(x_admin_key: str = Header(None, alias="X-Admin-Key")) -> str:
    """
    FastAPI Dependency – kiểm tra X-Admin-Key header cho các endpoint quản trị.
    Giống pattern đã dùng ở Recommendation Service v2.1.0.
    """
    if not ADMIN_API_KEY:
        # Nếu chưa cấu hình key → warning nhưng vẫn cho qua (dev mode)
        logger.warning("ADMIN_API_KEY chưa được cấu hình trong .env – bỏ qua auth check")
        return ""
    if not x_admin_key or x_admin_key != ADMIN_API_KEY:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="X-Admin-Key không hợp lệ. Endpoint này chỉ dành cho Admin.",
        )
    return x_admin_key
...
@router.post(
    "/rebuild-index",
    summary="Rebuild visual pHash index (Admin only)",
    description="Force rebuild perceptual hash index từ tất cả ảnh bìa sách trong MinIO. **Yêu cầu X-Admin-Key header.**",
)
async def rebuild_visual_index(admin_key: str = Depends(_verify_admin_key)):
    """Trigger rebuild pHash index (chạy background, không block request). Yêu cầu Admin Key."""
    import asyncio
...
# ══════════════════════════════════════════════════════════════════════════════
# ENDPOINT 0.6: INCREMENTAL INDEX – thêm/cập nhật 1 sách (Admin only)
# ══════════════════════════════════════════════════════════════════════════════

@router.post(
    "/index-book/{book_id}",
    summary="Thêm/cập nhật 1 sách vào visual pHash index (không rebuild toàn bộ)",
    description=(
        "Được gọi tự động từ Java Backend sau khi thêm hoặc sửa sách.\n\n"
        "Tải ảnh bìa từ MinIO → tính pHash → upsert vào index in-memory. "
        "Nhanh hơn rebuild-full ~1000x (chỉ xử lý 1 sách thay vì toàn bộ). "
        "**Yêu cầu X-Admin-Key header.**"
    ),
)
async def index_one_book(
    book_id: int,
    admin_key: str = Depends(_verify_admin_key),
):
    """
    Incremental upsert 1 sách vào pHash index.
    Sách chưa có ảnh (ví dụ vừa thêm chưa upload ảnh) sẽ bị skipped – không lỗi.
    """
    result = await upsert_book_in_hash_index(book_id)
    return result


@router.delete(
    "/index-book/{book_id}",
    summary="Xóa 1 sách khỏi visual pHash index",
    description=(
        "Được gọi tự động từ Java Backend sau khi xóa sách.\n\n"
        "**Yêu cầu X-Admin-Key header.**"
    ),
)
async def remove_one_book(
    book_id: int,
    admin_key: str = Depends(_verify_admin_key),
):
    """Xóa sách khỏi pHash index ngay lập tức (đồng bộ, < 10ms)."""
    result = remove_book_from_hash_index(book_id)
    return result
...
async def search_by_cover(
    ...
    # ── MD5 Cache Check (truoc ca garbage detection) ─────────────────────
    ...
    # ══ TẦNG 2: OCR Pipeline ═════════════════════════════════════════════════
    # Không khớp visual → chạy OCR dual-mode để chọn kết quả tốt nhất
    logger.info("🖼️  [2/4] Preprocessing ảnh dual-mode (normal + aggressive)...")
    try:
    ...
    logger.info("🔤 [3/4] Chạy OCR normal mode trước...")
    try:
        normal_result = await extract_text_async(preprocessed_img, image_type=image_type)
    except Exception as e:
    ...
    # Chỉ chạy aggressive mode nếu normal confidence THẤP (< 0.72)
    # → tránh tốn thêm 5-8s cho ảnh đã đọc tốt
    # → vẫn cải thiện accuracy cho bìa màu sắc phức tạp / font nghệ thuật
    _OCR_CONF_THRESHOLD = 0.72
    if normal_conf < _OCR_CONF_THRESHOLD:
        logger.info("   Normal conf=%.3f < %.2f → thử aggressive mode...",
                    normal_conf, _OCR_CONF_THRESHOLD)
        try:
            aggressive_img, _ = preprocess(image_bytes, aggressive=True)
            agg_result = await extract_text_async(aggressive_img, image_type="document")
            if (agg_result.get("confidence", 0) > normal_conf
                    and agg_result.get("text", "").strip()):
                ocr_result = agg_result
                logger.info("   Aggressive mode thắng (conf %.3f > %.3f)",
                            agg_result["confidence"], normal_conf)
            else:
                logger.info("   Aggressive không cải thiện (conf %.3f), giữ normal",
                            agg_result.get("confidence", 0))
        except Exception as _agg_err:
            logger.warning("   Aggressive mode lỗi, giữ normal: %s", _agg_err)
    else:
        logger.info("   Normal conf=%.3f đủ tốt, bỏ qua aggressive mode", normal_conf)
