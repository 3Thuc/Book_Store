...
async def process_for_stream(
    ...
    # ====== ULTRA-EARLY INVENTORY/ADMIN HARD BLOCK ======
    ...
    _INV_I = _re_i.compile(
        ...
        r"|100 cuon|them cuon|xa kho|kiem kho"
    ...
    if _INV_I.search(message) or _INV_I.search(_ai(message)):
    ...
    if intent == "store_info":
        _mls_si = message.lower()
        _aw_si = ["o dau", "ở đâu", "dia chi", "địa chỉ",
                  "nam o", "nằm ở", "vi tri", "vị trí", "cho nao", "chi nhanh"]
...
async def process(
    ...
    # ====== ULTRA-EARLY INVENTORY/ADMIN HARD BLOCK (process) ======
    # process() returns 3-tuple (answer, sources, btns)
    import re as _re_proc, unicodedata as _uc_proc
    def _ap(s): return _uc_proc.normalize("NFD", s.lower().replace("đ","d").replace("Đ","d")).encode("ascii","ignore").decode()
    _INV_P = _re_proc.compile(
        r"cap nhat|them vao kho|them hang|nhap kho|xuat kho"
        r"|100 cuon|them cuon|xa kho|kiem kho"
        r"|danh sach khach|khach hang cho giao|log lai|bao cao kho"
        r"|xoa cuon|xoa sach|rebuild|xuat file|quan tri"
    ...
    if _INV_P.search(message) or _INV_P.search(_ap(message)):
...
LUẬT: Chỉ nhắc đúng tên sách trong danh sách: {book_titles_str}. Không tự bịa thêm tên sách khác."""
            ...
            if not display_books:
                 return f"Xin lỗi, tôi không tìm thấy cuốn sách nào khớp với từ khóa '{raw_query}'. Bạn vui lòng thử tìm bằng từ khóa khác nhé!", ["opensearch:books", "mysql:books"], []
...
LUẬT: Chỉ dùng tên sách trong danh sách: {book_titles_str}. Không tự bịa tên sách khác."""
        ...
        if book_title:
            ...
            if book:
                ...
                if similar:
                    ...
                    return f"Xin lỗi, tôi không tìm thấy thông tin sách '{book_title}' trong hệ thống.", ["mysql:books"], []
        else:
            # Không có book_title gì hết → semantic search bằng toàn bộ message
            books = search_books_semantic(message, top_k=1)
            if not books:
                 return "Vui lòng cho tôi biết tên sách cụ thể bạn muốn xem.", [], []
            ctx   = _format_book_detail(books[0])
    ...
    if intent == "book_compare":
        ...
        if not hits or len(hits) < 2:
            return "Xin lỗi, tôi không tìm thấy đủ dữ liệu sách để thực hiện so sánh theo yêu cầu của bạn.", ["opensearch:books"], []
        ctx  = "So sánh 2 cuốn:\n" + _format_books(hits)
    ...
    if intent == "book_availability":
        ...
        if book:
            ...
            return f"Xin lỗi, tôi không tìm thấy sách '{book_title}' trong hệ thống lúc này.", ["mysql:books"], []
    ...
    if intent == "book_review":
        ...
        if not display_books:
            return "Xin lỗi, tôi không tìm thấy cuốn sách này để gợi ý đánh giá.", ["opensearch:books"], []
            
        ctx   = _format_books(display_books)
    ...
    if intent == "recommend_personal":
        if not user_id:
            ...
            display_books = _filter_and_track_books(books, context, max_items=4)
            if not display_books:
                return "Xin lỗi, hiện tại tôi chưa tìm thấy sách nào phù hợp để gợi ý cho bạn. Bạn thử xem danh mục sách bán chạy nhé!", ["opensearch:books"], []
        ...
        display_books = _filter_and_track_books(books, context, max_items=4)
        if not display_books:
            return "Xin lỗi, hiện tại tôi chưa tìm thấy sách nào phù hợp để gợi ý cho bạn. Bạn thử xem danh mục sách bán chạy nhé!", ["mysql:books"], []
    ...
    if intent == "recommend_trending":
        ...
        display_books = _filter_and_track_books(books, context, max_items=4)
        if not display_books:
            return "Xin lỗi, danh sách xu hướng hiện tại đang trống. Bạn muốn tìm sách theo từ khóa nào không?", ["opensearch:books"], []
    ...
    if intent == "order_status":
        ...
        if order_id:
            ...
            if not order:
                return f"Xin lỗi, tôi không tìm thấy thông tin đơn hàng #{order_id}. Vui lòng kiểm tra lại mã đơn nhé.", ["mysql:orders"], []
            ctx   = _format_order(order)
        elif user_id:
            orders = get_user_orders(user_id, limit=3)
            if not orders:
                return "Theo hệ thống ghi nhận, bạn chưa có đơn hàng nào cả. Bạn có muốn tìm sách mới để đặt mua không?", ["mysql:orders"], []
    ...
    if intent == "cart_help":
        ...
        cart_btn = NavigateButton(label="🛒 Xem giỏ hàng", url="/cart", type="page")
        return answer, ["opensearch:kb"], [cart_btn]
    ...
    # == image_search: OCR result -> search book -> return buttons ==
    if intent == "image_search":
        # Extract OCR-parsed title from message (format: "📷 Tìm sách từ ảnh: \"Title\"")
        _ocr_title = None
        ...
        if _ocr_title and len(_ocr_title) > 3:
            _ocr_books = search_books_semantic(_ocr_title, top_k=4)
            if not _ocr_books:
                _ocr_books = search_books_semantic(message, top_k=4)
            _ocr_display = _filter_and_track_books(_ocr_books, context, max_items=4)
            if _ocr_display:
                context["last_found_title"] = _ocr_display[0].get("title", "")
                context["last_category"] = _ocr_display[0].get("category", "")
                _ocr_ctx = f"[H\u1ec6 TH\u1ed0NG]: T\u00ecm th\u1ea5y {len(_ocr_display)} s\u00e1ch li\u00ean quan: " + ", ".join(f'"{b.get("title","")}"'  for b in _ocr_display)
                _ocr_ans = await generate(message, _ocr_ctx, history, tone, intent="book_search")
                return _ocr_ans + FOLLOW_UP.get("book_search", ""), ["opensearch:books"], _make_book_buttons(_ocr_display)
        # OCR returned nothing useful - fall through to SMART FALLBACK
        # but still try a semantic search of full message
        _ocr_fallback = search_books_semantic(message, top_k=4)
        _ocr_fb_disp = _filter_and_track_books(_ocr_fallback, context, max_items=4)
        if _ocr_fb_disp:
            _ocr_fb_ctx = f"[H\u1ec6 TH\u1ed0NG]: S\u00e1ch li\u00ean quan: " + ", ".join(f'"{b.get("title","")}"'  for b in _ocr_fb_disp)
            _ocr_fb_ans = await generate(message, _ocr_fb_ctx, history, tone, intent="book_search")
            return _ocr_fb_ans + FOLLOW_UP.get("book_search", ""), ["opensearch:books"], _make_book_buttons(_ocr_fb_disp)
        # Nothing found at all
        return "\u0110ang nh\u1eadn d\u1ea1ng... R\u1ea5t ti\u1ebfc t\u00f4i ch\u01b0a t\u00ecm th\u1ea5y s\u00e1ch ph\u00f9 h\u1ee3p t\u1eeb h\u00ecnh \u1ea3nh n\u00e0y. B\u1ea1n c\u00f3 th\u1ec3 cho t\u00f4i bi\u1ebft t\u00ean s\u00e1ch kh\u00f4ng?", [], []
    ...
    if intent == "out_of_scope":
        ...
        if any(_msg_oos == kw or _msg_oos.startswith(kw + " ") or _msg_oos.endswith(" " + kw) for kw in _farewell_kw):
            if any(w in _msg_oos for w in ("cam on", "thanks", "thank")):
                return TEMPLATES.get("thanks_reply", "Kh\u00f4ng c\u00f3 g\u00ec!"), [], []
            if any(w in _msg_oos for w in ("bye", "tam biet", "thoat", "logout", "hen gap")):
                return TEMPLATES.get("farewell", "H\u1eb9n g\u1eb7p l\u1ea1i!"), [], []
...
async def _handle_slot_filling_response(
    ...
    if _new_intent != target_intent and _new_intent in _clear_intents and _new_nlu.confidence >= 0.6:
